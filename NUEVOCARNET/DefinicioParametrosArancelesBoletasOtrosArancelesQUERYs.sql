...
select dtde_codigo, dtde_nombre from col_dtde_detalle_tipo_estudio
alter procedure sp_detalle_tipo_estudio_alumno
	-- =============================================
	-- Author:      <Fabio>
...
select * from col_tmo_tipo_movimiento where tmo_codigo in (852  ,909  ,1043 ,128  ,3245 );
select * from col_dpboa_definir_parametro_boleta_otros_aranceles
...
values 
(852 , 1,  1, '07/06/2019', 1, '16/01/2019', '08/06/2019', 1, 0, 1, 1),
(909 , 1,  0, '', 1, '01/02/2019', '31/05/2019', 1, 0, 2, 1),
(1043, 0,  0, '', 0, '', '', 1, 0, 1, 1),
(128 , 0,  0, '' , 1, '29/05/2019', '31/05/2019', 1, 0, 2, 1),
(3245, 0,  1, '05/06/2019', 1 ,'28/05/2019', '08/06/2019', 1, 0, 2, 1)
--select * from col_dpboa_definir_parametro_boleta_otros_aranceles

select   dpboa_codigo 'coddpboa', tmo_arancel 'Arancel', dpboa_codtmo, case dpboa_afecta_materia when 1 then 'Si' else 'No' end 'Afecta materia', case dpboa_cupo_vencimiento when 1 then 'Si' else 'No' end 'Cupo vencimiento', dpboa_fecha_vencimiento 'Fecha vencimiento', case dpboa_periodo when 1 then 'Si' else 'No' end 'Tiene periodo', dpboa_fecha_desde 'Fecha desde', dpboa_fecha_hasta 'Fecha hasta', tmo_cupo 'Cupo', dpboa_estado from col_dpboa_definir_parametro_boleta_otros_aranceles inner join col_tmo_tipo_movimiento on dpboa_codtmo = tmo_codigo
select dpboa_codigo, dpboa_codtmo, dpboa_afecta_materia, dpboa_cupo_vencimiento, dpboa_fecha_vencimiento, dpboa_periodo, dpboa_fecha_desde, dpboa_fecha_hasta, tmo_arancel, tmo_cupo from col_dpboa_definir_parametro_boleta_otros_aranceles inner join col_tmo_tipo_movimiento on dpboa_codtmo = tmo_codigo where dpboa_codtmo = 852

select dpboa_codigo, dpboa_codtmo, dpboa_afecta_materia, dpboa_cupo_vencimiento, dpboa_fecha_vencimiento, dpboa_periodo, dpboa_fecha_desde, dpboa_fecha_hasta, tmo_arancel, tmo_cupo, dpboa_estado, dpboa_afecta_evaluacion, dpboa_codta from col_dpboa_definir_parametro_boleta_otros_aranceles inner join col_tmo_tipo_movimiento on dpboa_codtmo = tmo_codigo

select dpboa_codtmo, concat(tmo_arancel, ' ', tmo_descripcion) from col_dpboa_definir_parametro_boleta_otros_aranceles inner join col_tmo_tipo_movimiento on tmo_codigo = dpboa_codtmo where dpboa_estado = 1
...
select * from col_dao_data_otros_aranceles, col_dpboa_definir_parametro_boleta_otros_aranceles, col_tmo_tipo_movimiento where dao_coddpboa = dpboa_codigo and dpboa_codtmo = tmo_codigo
...
ALTER PROCEDURE [dbo].[sp_datos_alumno_codigo_barra]
	...
	-- Description:	<>
	-- =============================================
	--	sp_datos_alumno_codigo_barra 1,'415741970000313739020000004500960000000880202515652015*012019',8
	--	sp_datos_alumno_codigo_barra 2,'415741970000313739020000004500960000005580202515652015*012019',8
	@opcion int = 0,
	@codigo_barra nvarchar(80) = '', 
	@codigo_banco int = 0
AS
BEGIN
	declare @codper int
	...
	if @opcion = 1
	begin
		select @dao_codigo =  substring(@codigo_barra,33,8)
		...
		print '-----------------------------------------'
		print 'Estado : -1 ARANCEL NO EXISTE EN LA TABLA dpboa'
	...
	begin
		select @dao_codigo =  substring(@codigo_barra,33,8)
		print '@dao_codigo ' + cast(@dao_codigo as varchar(255))
		select @codper = dao_codper from col_dao_data_otros_aranceles where dao_codigo = @dao_codigo
		select @arancel_codigo = dpboa_codtmo from col_dao_data_otros_aranceles 
		join col_dpboa_definir_parametro_boleta_otros_aranceles on dpboa_codigo = dao_coddpboa
		where dao_codigo = @dao_codigo
...
ALTER procedure [dbo].[sp_reversion_pago_codigo_barra]
	...
	-- Description:	<>
	-- =============================================
	-- sp_reversion_pago_codigo_barra '415741970000313739020000000700960000000180202515652015*012019', 11, '2287193'
	@barra varchar(100),
	@tipo int,
 ...
 begin
	...
	from col_dao_data_otros_aranceles, ra_per_personas 
	where dao_barra = @barra and dao_codper = per_codigo
...
alter procedure [dbo].[sp_encabezado_detalle]
	...
	-- Description: <Devuelve el encabezado o el detalle de un pago para el webservice del banco azul> 
	-- =============================================
...
ALTER PROCEDURE [dbo].[sp_insertar_encabezado_pagos_online_otros_aranceles]
	-- =============================================
	-- Author:		<Juan Carlos Campos Rivera>
	-- Create date: <Viernes 31 Mayo 2019>
	-- Description:	<Aplicar pagos de otros aranceles en linea>
	-- =============================================
	--	exec sp_insertar_encabezado_pagos_online_otros_aranceles '4157419700003137390200000045009600000018802025071320169012019', 12, '34kj78u25414'
	--	select * from col_dpboa_definir_parametro_boleta_otros_aranceles
	-- sp_helptext sp_insertar_encabezado_pagos_online_otros_aranceles
	@barra varchar(80),
	@tipo int,
...
BEGIN
	...
	begin
		...
		/*Variables para el detalle de factura */
			...
			exec col_efpc_EncabezadoFacturaPagoCuotas
			--select 
	...
	--	--values(@codper, @npe, @cil_codigo,ERROR_MESSAGE() + ' ,' + @Mensaje,ERROR_NUMBER(),ERROR_severity(),error_state(),ERROR_procedure(),ERROR_line())
