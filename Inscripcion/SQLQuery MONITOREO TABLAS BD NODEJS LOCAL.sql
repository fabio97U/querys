...
SELECT fac, count(1) 'pagos_ra_validac', count(ins_codigo) 'inscritos', count(1) - count(ins_codigo)  'restantes' FROM (
	...
	left join dbo.ra_ins_inscripcion on ins_codper = per_codigo and ins_codcil = 126
) T
group by fac
--12595
...
inner join ra_per_personas on per_codigo = ins_codper
where ins_codigo not in (select mai_codins from dbo.ra_mai_mat_inscritas) and ins_codcil = 126
and per_tipo = 'U'
...
where mai_codins not in (select ins_codigo from dbo.ra_ins_inscripcion where ins_codcil = 126)
--
...
select * from dbo.ra_mai_mat_inscritas where mai_codins in (1265321)
