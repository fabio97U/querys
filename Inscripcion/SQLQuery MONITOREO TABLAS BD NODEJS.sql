exec dbo.web_ptl_login_ldap '1109022015', 125, 125
exec dbo.web_ptl_login_info_fqdn '1109022015'
-- exec dbo.web_ins_genasesoria_con_matins_nodjs_azure 125, 216122		--hoja_inscripcion
...
-- exec dbo.web_ins_matinscritas_nodjs 228835, 125				--comprobante de materias
--1222723, 4969582
...
where ins_codcil = 125 and ins_codper = 218834
...
select * from ins_errins_errores_inscrpcion with (nolock) where errins_codcil = 125
...
SELECT fac, count(1) 'pagos_ra_validac', count(ins_codigo) 'inscritos' FROM (
	...
	left join dbo.ra_ins_inscripcion on ins_codper = per_codigo and ins_codcil = 125
) T
group by fac
--12595
...
where mai_codpla <> alc_codpla and per_tipo = 'U' and ins_codcil = 125 --and alc_codper = 224535
...
--exec web_ins_genasesoria 125, 209505
...
where ins_codigo not in (select mai_codins from dbo.ra_mai_mat_inscritas) and ins_codcil = 125
...
where mai_codins not in (select ins_codigo from dbo.ra_ins_inscripcion where ins_codcil = 125)


--MAS DE 5 CODMAI INSCRITAS
select ins_codigo, count(1) from dbo.ra_ins_inscripcion
inner join dbo.ra_mai_mat_inscritas on mai_codins = ins_codigo
where ins_codcil = 125 and ins_usuario_creacion = 'user.online'
group by ins_codigo
having count(1) > 5
...
inner join ra_per_personas on per_codigo = ins_codper
inner join ra_alc_alumnos_carrera on alc_codper = per_codigo
where ins_codigo in (1234465, 1234709)
order by ins_codigo

select * from dbo.ra_mai_mat_inscritas where mai_codins in (1246669, 1234709)
