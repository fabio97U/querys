exec dbo.web_ptl_login_ldap '1109022015', 129, 129
exec dbo.web_ptl_login_info_fqdn '1109022015'
-- exec dbo.web_ins_genasesoria_con_matins_nodjs_azure 129, 216122		--hoja_inscripcion
...
-- exec dbo.web_ins_matinscritas_nodjs 228835, 129				--comprobante de materias
--1222723, 4969582
...
where ins_codcil = 129 --and ins_codper = 186836
...
select * from ins_errins_errores_inscrpcion with (nolock) where errins_codcil = 129 order by errins_fecha_creacion
...
SELECT fac, count(1) 'pagos_ra_validac', count(ins_codigo) 'inscritos', count(1) - count(ins_codigo)  'restantes' FROM (
	...
	left join dbo.ra_ins_inscripcion on ins_codper = per_codigo and ins_codcil = 129
) T
group by fac
--12595
...
where mai_codpla <> alc_codpla and per_tipo = 'U' and ins_codcil = 129 --and alc_codper = 224535
...
--exec web_ins_genasesoria 129, 209505
...
where ins_codigo not in (select mai_codins from Inscripcion.dbo.ra_mai_mat_inscritas) and ins_codcil = 129
...
where mai_codins not in (select ins_codigo from Inscripcion.dbo.ra_ins_inscripcion where ins_codcil = 129)
--
--MAS DE 5 CODMAI INSCRITAS
select ins_codigo, ins_codper, count(1) from Inscripcion.dbo.ra_ins_inscripcion
inner join Inscripcion.dbo.ra_mai_mat_inscritas on mai_codins = ins_codigo
where ins_codcil = 129 and ins_usuario_creacion = 'user.online'
